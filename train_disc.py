...
from utils import (save_args_to_yaml,
                   ...
                   reNormalize_img, 
                   normalize_mean_std)
from src.discriminator import PatchDiscriminator, discriminator_loss, generator_gan_loss
...
def main():
    ...
    model, discriminator, optimizer, d_optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        model, discriminator, optimizer, d_optimizer, train_dataloader, lr_scheduler)
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, samples in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                discriminator.train()
                real_logits = discriminator(nonorm_target_images)
                fake_logits = discriminator(pred_original_sample.detach())
                d_loss = discriminator_loss(real_logits, fake_logits, mode='hinge')
                ...
                # Generator GAN loss
                g_loss = generator_gan_loss(discriminator(pred_original_sample), mode='hinge')
                                
                loss = diff_loss + \
                        args.perceptual_coefficient * percep_loss + \
                            args.offset_coefficient * offset_loss + args.gan_coefficient * g_loss                
                if args.phase_2:
